...
def verify(addr, local_addr='john.smith@example.com', smtp_server=''):
  ...
  if len(smtp_server) > 0:
    user, pword, host = smtp_server.split(':')
  else:
  ...
  code, response = server.docmd('mail from: ', '<%s>' % local_addr)
  logging.debug('MAIL FROM command returned %d: %r', code, response)
  code, response = server.docmd('rcpt to: ', '<%s>' % addr)
...
def main(*args, **opts):
  ...
  for addr in args:
    if verify(addr, local_addr=opts['local_addr'], smtp_server=opts['smtp_server']):
      print '%r exists.' % addr
    else:
